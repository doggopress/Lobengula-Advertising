...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $mail->Host       = 'mail.lobengula.co.za';                    // Set the SMTP server to send through
        $mail->SMTPAuth   = true;                                   // Enable SMTP authentication
        $mail->Username   = 'noreply@lobengula.co.za';                     // SMTP username
        $mail->Password   = '6Z&,tV}R2-Y%';                               // SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
        $mail->Port       = 465;                                    // TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above

        //Recipients
        $mail->setFrom('noreply@lobengula.co.za', 'Lobengula Advertising');
        $mail->addAddress($_POST['email'], $_POST['name']);     		
        $mail->addReplyTo('shirley@lobengula.co.za', 'Lobengula Advertising');
        $mail->addBCC('brenda@lobengula.co.za', 'Lobengula Advertising');
